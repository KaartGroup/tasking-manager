...
def is_known_editor(value):
    ...
    try:
        ...
        raise ValidationError(f'Unknown editor: {value} Valid values are {Editors.ID.name}, '
                              ...
                              f'{Editors.FIELD_PAPERS.name}, {EDITORS.ID_X.name}')
...
class ProjectDTO(Model):
    ...
    map_editors = ListType(StringType, serialized_name='mapEditors', validators=[is_known_editor])
    validate_editors = ListType(StringType, serialized_name='validateEditors', validators=[is_known_editor])
...
class ProjectSearchDTO(Model):
    ...
    map_editors = ListType(StringType, validators=[is_known_editor])
    validate_editors = ListType(StringType, validators=[is_known_editor])
    ...
    def __hash__(self):
        ...
        hashable_map_editors = ''
        if self.map_editors:
            for map_editor in self.map_editors:
                hashable_map_editors = hashable_map_editors + map_editor
        hashable_validate_editors = ''
        if self.validate_editors:
            for validate_editor in self.validate_editors:
                hashable_validate_editors = hashable_validate_editors + validate_editor

        return hash((self.preferred_locale, self.mapper_level, hashable_mapping_types, hashable_project_statuses,
                     self.organisation_tag, self.campaign_tag, self.page, self.text_search, self.is_project_manager, hashable_map_editors, hashable_validate_editors))
