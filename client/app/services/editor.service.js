(function () {
    ...
    function editorService($window, $location, $q, mapService, configService) {
        ...
        /**
         * Launch the iD X editor
         * TODO: Update URL and parameters for iD X when they're finalized
         * @param centroid
         * @param changesetComment
         * @param imageryUrl
         * @param projectId
         * @param taskId
         */
        function launchIdX(centroid, changesetComment, imageryUrl, projectId, taskId){
            var base = 'http://www.openstreetmap.org/edit?editor=id&';
            var zoom = mapService.getOSMMap().getView().getZoom();
            var url = base + '#map=' +
                        [zoom, centroid[1], centroid[0]].join('/');
